...
public static class NIGUI 
{
    ...
    public static bool RepeatButton(Rect pos, string text)
    {
        ...
        if (m_cursor.HasClickedThisFrame() == false  && (!m_cursor_add || m_cursor_add.HasClickedThisFrame() == false))
            return false;
        //if(m_cursor_add)
        ...
        bool clicked = false;
        if (m_cursor.HasClickedThisFrame())
            clicked = pos.Contains(m_cursor.Position);
        else if (m_cursor_add && m_cursor_add.HasClickedThisFrame())
            clicked = pos.Contains(m_cursor_add.Position);
    ...
    //MI: repeatbuttonrest
    private static bool RepeatButtonRest(Rect pos)
    {
        if (m_cursor == null)
            throw new System.Exception("You must set a NIGUICursor using NIGUI.SetCursor(newCursor).");
        if (m_cursor.HasClickedThisFrame() == false && (!m_cursor_add || m_cursor_add.HasClickedThisFrame() == false))
    ...
    static private bool TestClicked(ref Rect where, out Vector2 lastClickPosition)
    {
        ...
        int lastClickFrame_add ;
        Vector2 lastClickPosition_add;
        if (m_cursor_add)
        {
            float lastClickTime_add = m_cursor_add.GetLastClickedTime(out lastClickFrame_add, out lastClickPosition_add);
            if (lastClickTime_add > lastClickTime)
            {
                lastClickTime = lastClickTime_add;
                lastClickFrame = lastClickFrame_add;
                lastClickPosition = lastClickPosition_add;
            }
        }
    ...
    static private NIGUICursor m_cursor_add = null;
